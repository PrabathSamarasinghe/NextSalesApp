...
export default function ReceivedInvoiceEntry(): JSX.Element {
  ...
  const [suppliers] = useState<{_id: string, name: string}[]>([{_id: "1", name:"Devagiri Tea Factory"}]);
  ...
  const removeItem = (id: number): void => {
    if (receivedInvoice.items.length > 1) {
      const filteredItems = receivedInvoice.items.filter((item) => item.id !== id);
      setReceivedInvoice({
        ...receivedInvoice,
        items: filteredItems,
      });
  ...
  const calculateTotal = (): number => {
    const total = receivedInvoice.items.reduce((sum, item) => sum + item.total, 0);
    return total;
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow-lg border border-gray-100">
          <div className="flex justify-between items-center px-6 py-5 border-b border-gray-200 bg-gray-50 rounded-t-lg">
            <h2 className="text-2xl font-bold text-gray-800">New Received Invoice</h2>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setReceivedInvoice({ ...receivedInvoice, invoiceNumber: e.target.value })
                  }
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setReceivedInvoice({ ...receivedInvoice, date: e.target.value })
                  }
                  className="w-full p-2.5 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
              ...
              <div className="w-full md:w-1/2">
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setReceivedInvoice({ ...receivedInvoice, notes: e.target.value })
                    }
                  ></textarea>
                </div>
              ...
              <div className="w-full md:w-1/2 lg:w-1/3">
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                  ...
                  <div className="flex justify-between py-2 text-sm">
                    ...
                    <span className="text-gray-700">{receivedInvoice.items.length}</span>
                  </div>
                  <div className="flex justify-between py-2 text-sm">
                    <span className="text-gray-700">Total Quantity:</span>
                    <span className="text-gray-700">
                      {receivedInvoice.items.reduce((sum, item) => sum + item.quantity, 0)}
                    </span>
                  </div>
                </div>
              </div>
